...
**The enforcement locus is a separable layer (the future-proofing), and it never becomes a gate on exchange.** The token is, and remains, a **plain ERC-1155** — minted once, valid under every phase of this design. Binding state lives in an **on-chain, world-readable binding registry from launch** — a separate accounting contract — so a unit's released status is publicly verifiable without trusting a private ledger. **Its writers are the compliant players themselves:** a bind or release is accepted only when it carries a valid, non-revoked compliant-device attestation authorizing that operation plus the owner's wallet signature, and the registry contract enforces `boundCount ≤ balance` at write time by reading the token's `balanceOf`. **No central issuer writes the flag** — so released status can never be withheld by a single party, from launch. The only non-device writer is the wallet owner, who forces a recovery-release after the report window (a device can never veto its owner). The key issuer (§4) is confined to *key wrapping* — enabling decryption at first bind — which is separate from flag state and decentralizes on its own path (§5). What does **not** exist in any phase is a power to block a transfer: **the registry *informs*; it never *gates*.** The token transfers freely on-chain always — the protocol lets no party, issuer included, block the exchange of an owned token. (A contract-level transfer rule is conceivable only as an *opt-in*, per-title property a publisher could elect — never a protocol-wide veto; open question (§9), not a committed step.) Registry writes verify a device credential on-chain against the recognized-roots / revocation registry (§4) — a real gas and engineering cost, flagged as an implementation item. The *semantics* are fixed now, so **tokens minted at launch carry forward unchanged.**
...
3. **The player writes its own bind.** The **registry contract** verifies the attestation and the wallet signature, checks `boundCount < balance` against the token's `balanceOf`, and **increments `boundCount`** — only a compliant player, authorized by the owner's wallet, can set the flag. Separately, the **key issuer** verifies the same attestation and **wraps the master key to the player's secure-element public key**; a revoked player (§4) is refused at both. Flag write and key wrap are distinct acts: the fleet-written registry records the bind; the issuer only enables decryption.
...
- **Clean release** (device present and cooperating): the device deletes its wrapped key and **attests the deletion**; `boundCount` decrements immediately, no residual. This is the normal path — release before you list, or before you sell/gift the hardware.
- **Reported lost, sold, or broken** (no clean release possible): the wallet owner submits a signed **report**; **30 days** later (a tunable parameter) the titles bound to that device are released and the units become transferable. The device, whenever it next transacts on the network, must **verifiably erase** the released titles as a **precondition of that transaction** (its next download/bind is refused until it does); a device that refuses is **revoked** (§4) and locked out. An *actively used* device therefore cannot retain released titles while staying online for new content.
...
### 2.1 Binding and the rights tiers {-}

Personal viewing binds one copy against `balanceOf ≥ 1`. Exhibition tiers raise the bar: a tier-*T* copy's release condition is `balanceOf ≥ threshold_T`, and binding a tier-*T* copy holds `threshold_T` units against `boundCount` for as long as it is bound (a venue's staked exhibition units are *in use*, not merely owned, while a screening capability is live). The exact accounting for overlapping tiers, and how staking feeds the per-tier condition, is specified in the entitlement contract (`PROTOCOL_LAYERS.md` §5) and is an open item where stacking and binding interact (§9).
