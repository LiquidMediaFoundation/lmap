...
**The enforcement locus is a separable layer (the future-proofing), and it never becomes a gate on exchange.** The token is, and remains, a **plain ERC-1155** — minted once, valid under every phase of this design. Binding state lives in an **on-chain, world-readable binding registry from launch**, so a unit's released status is publicly verifiable without trusting a private ledger. What migrates is *who may write that registry*: at launch the issuer (§4) is the sole writer — it increments on bind, decrements on attested release, and refuses to wrap beyond `balance` — and over time that write authority decentralizes (single issuer → federated anchors → threshold network), so released status can no longer be withheld by any single party. What does **not** migrate is any power to block a transfer: **the registry *informs*; it never *gates*.** The token transfers freely on-chain in every phase — the protocol lets no party, issuer included, block the exchange of an owned token. (A contract-level transfer rule is conceivable only once the registry's writer is trustless, and even then would be an *opt-in* property a publisher could elect per title — never a protocol-wide veto; it is an open question (§9), not a committed step.) The *semantics* are fixed now; the *trust in the writer* decentralizes underneath, so **tokens minted at launch carry forward unchanged.**

**Binding flow (once, at acquisition):**
1. The buyer's wallet acquires the token.
2. The compliant player presents to the **issuer** (§4): (a) a **device attestation report** proving it is a certified, integrity-checked direct player with a secure element, and (b) a fresh **wallet-signed bind authorization** naming the device, proving the wallet currently owns an unbound unit.
3. The issuer verifies both, checks `boundCount < balance`, **increments `boundCount`**, and **wraps the master key to the player's secure-element public key**. A revoked player (§4) is refused.
