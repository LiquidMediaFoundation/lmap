...
**Recovery is wallet-authoritative, never device-authoritative.** Playback always fails *open* — a present player keeps playing its bound copies forever — so only *transferability* depends on liveness, and the **wallet is the root of ownership**: whoever proves control of the wallet on-chain may reclaim a unit's transferability. A device can never veto its own owner. Three paths:
- **Clean release** (device reachable): the device attests deletion; `boundCount` decrements immediately, no residual.
- **Lost/dead device** (no attestation possible): after a **30-day** heartbeat silence (a tunable parameter), the owner force-releases without a deletion attestation. The 30-day cost — the device is dark and unwatchable throughout — is what makes this a genuine recovery, not a frictionless double-dip.
- **Stolen or hostile online device** (won't release, keeps heartbeating): the owner **reports it compromised**, revoking it (§4); a revoked device's heartbeat is disregarded for recovery, so an adversary cannot hold an owner hostage.

The residual in the latter two paths — a force-released device that retains a playable copy — is watermarked and personally attributable (§7); a false theft report is a logged, fraud-flavored act. This dissolves the theft, gift, inheritance, and hostile-previous-holder deadlocks that a device-authoritative rule creates. The precise force-release rate-limiting and dispute governance are an open spec item (§9).
