...
**The enforcement locus is a separable layer (the future-proofing).** The token is, and remains, a **plain ERC-1155** — minted once, valid under every phase of this design. Only *where the `boundCount` rules are enforced* migrates: at launch, **issuer-gated** — the issuer (§4) holds `boundCount` off-chain and refuses to wrap beyond `balance`; then an on-chain **binding registry** the issuer's state migrates into, making binding trustless and publicly verifiable with no change to the token; ultimately, contract-level transfer *reverts* via a `_beforeTokenTransfer` hook (the one step that needs a hook-capable token — §5, §8). The *semantics* are fixed now; the *trust* decentralizes underneath, so **tokens minted at launch migrate up for free.**
...
**Transfer and release.** A holder transfers only *released* units. To release, the bound player deletes its wrapped key and **attests the deletion** to the issuer, which **decrements `boundCount`**. A protocol sale settles through a marketplace contract that verifies — from an **issuer-signed attestation** (at launch) or the on-chain registry (post-migration) — that the seller holds `≥ N` released units, **atomically** with the transfer, payment, and royalty, so a buyer can never pay for a unit whose copy is still bound. The norm is therefore *release before you list*. **Honest boundary:** at launch the token transfers freely on-chain while `boundCount` lives off-chain, so a **raw** transfer *outside* the marketplace can move a still-bound unit and momentarily break the invariant (the seller keeps a playing copy; the buyer binds another). This is detectable — the issuer reconciles `boundCount > balance` on transfer — forensically watermarked, and bounded to deliberate out-of-protocol behavior: the physical-media-tier residual that migration step C (the on-chain hook) closes outright. (Optional fast-follow: *atomic release-at-sale* bundles release into settlement so a seller can watch until the moment of sale.)
